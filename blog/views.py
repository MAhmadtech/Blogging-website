...
def post_list(request, tag_slug=None):
    post_list = Post.published.all()  # Changed variable name to avoid conflict
    tag = None
    if tag_slug:
...
def post_detail(request, year, month, day, post):
    ...
    comments = post.comments.filter(active=True)  # Changed from post.comment to post.comments
    form = CommentForm()
...
def post_share(request, post_id):
 ...
 # Form fields passed validation
            ...
            send_mail(subject, message, 'ahmadbadar3635@gmail.com',
                      [cd['to']])
            sent = True
